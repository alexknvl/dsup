...
def main3(args):
    ...
    for (path, attr, hash) in dirs:
        if os.path.exists(os.path.join(args.outputdir, attr + '_' + hash)):
            continue

        predictions = result_store.v2_read_predictions(path, 'test')
        ...
        for p in predictions:
            ...
            month_bins.setdefault(ym, []).append(p)

        def check_edits(p):
            edits = p.s.edits
            if len(edits) != 0:
                date = easytime.timestamp_to_datetime(p.s.timestamp, 'utc')
                edit_date = easytime.timestamp_to_datetime(
                    min(e.timestamp for e in edits), 'utc')
                if (edit_date.year, edit_date.month) < (date.year, date.month):
                    return False
            return True

        def metric(a, b):
            r = ep_metric(a.s.args, b.s.args)
            if r >= 3:
                return r
            else:
                return 0
        ...
        for date, predictions in month_bins.iteritems():
            # Select only the best result per entity pair.
            by_ep = fx.groupby(predictions, key=lambda t: t.s.args)
            for k, v in by_ep.iteritems():
                v = fx.max_by(v, key=lambda t: t.p)
                by_ep[k] = v
            ...
            top = select_top_fuzzy(
                sorted(by_ep.itervalues(), key=lambda t: -t.p),
                at_least=20, at_most=50,
                metric=metric,
                predicate=lambda t: True,
                score=lambda t: t.p,
                min_score=0.9 if params['mode'] == 'baseline' else 0.3)
            month_bins[date] = top

        with open(os.path.join(args.outputdir, attr + '_' + hash), 'w+') as f:
            for date, predictions in sorted(month_bins.items()):
                for p in predictions:
                    f.write(attr + '\t')
                    f.write(params['mode'] + '\t')
                    f.write(' '.join(str(s) for s in date) + '\t')
                    f.write(str(p.p) + '\t')
                    f.write(str(p.y) + '\t')
                    f.write(str(p.s.id) + '\t')
                    f.write(str(p.s.args[0]) + '\t')
                    f.write(str(p.s.args[1]) + '\t')
                    f.write(' '.join(p.s.text.words) + '\n')

        # out = open(os.path.join(args.outputdir, attr + '_' + hash), 'w+')
        # shuffled = fx.shuffle(p for date, predictions in month_bins.iteritems()
        #                       for p in predictions)
        # for p in shuffled:
        #     out.write('\t'.join(str(s)
        #               for s in [p.s.id, p.s.args[0], p.s.args[1], p.s.text]) + '\n')
        # out.close()
