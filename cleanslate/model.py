...
def main(args):
    event_mapping = data.read_event_mapping_file(
        '/home/konovalo/dsup_event/new-data/event-wiki-mapping.json')
    ...
    event_mapping = sorted(event_mapping,
                           key=lambda t: 0 if t.name in attrs else 1)

    attrs = set([
        #'Spouse',
        # 'Awarded',
        # 'SocialEventIn', 'Awarded',
        # 'CollaborationWith', 'DeathPlace',
        'OldLeaderName',
        #    'CurrentTeam',
        # 'NewProduct', 'Developer'
    ])
    # event_mapping = [e for e in event_mapping if e.name in attrs]

    heval_results = list(data.read_mturk_dev_file(
        '/home/konovalo/dsup_event/new-data/gen/mturk-dev-new.json'))

    annotated_ids = set(list(
        line.strip()
        for line in open('/home/konovalo/dsup_event/new-data/annotated_ids.csv')))

    well_aligned = (
        line.strip().split('\t')
        for line in open('/home/konovalo/dsup_event/new-data/gen/well-aligned-train-args.csv'))
    well_aligned = fx.groupby(well_aligned, key=lambda t: t[0], map=lambda t: t[1:])
    for k in well_aligned:
        well_aligned[k] = fx.groupby(
            well_aligned[k],
            key=lambda t: (t[0], t[1]),
            map=lambda t: (float(t[2]), float(t[3])))
        for a in well_aligned[k]:
            well_aligned[k][a] = well_aligned[k][a][0]
