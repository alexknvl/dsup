...
class LR:
    ...
    def GRAD(self,w,X,Y):
        Y = (Y+1.0) / 2
        #print "test: %s" % type(((1.0 / (1.0 + np.exp(-np.dot(w,X.T)))) - Y))
#        print "1: %s" % ((1.0 / (1.0 + np.exp(-np.dot(w,X.T)))).shape)
#        print "2: %s" % str(np.dot(X.T, (1.0 / (1.0 + np.exp(-np.dot(w,X.T)))) - Y).shape)
#        print "3: %s" % str((np.dot(X.T, (1.0 / (1.0 + np.exp(-np.dot(w,X.T)))) - Y) + 2 * self.l2 * w).shape)
        return np.dot(X.T, (1.0 / (1.0 + np.exp(-np.dot(w,X.T)))) - Y) + 2 * self.l2 * w
